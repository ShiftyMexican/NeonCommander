...
namespace TestGameOne
{
    ...
    public class ConceptBullets
    {
        ...
        public ConceptBullets(Vector2 position, float rotation, Vector2 facingDir)
        {
            ...
            m_line = AIE.GameStateManager.Game.Content.Load<Texture2D>("Images/ball");
            m_deathExplosion = Particles.ParticleManager.CreateParticleSystem(m_line);

            m_deathExplosion.rate = 0.01f;
            m_deathExplosion.startAlpha = new Particles.Range(0.3f, 0.4f);
            m_deathExplosion.endAlpha = new Particles.Range(0.0f, 0.0f);
            m_deathExplosion.startColor = Color.Gray;
            m_deathExplosion.endColor = Color.Black;
            m_deathExplosion.normColor = 0.5f;

            m_deathExplosion.spawnArea = 3f;
            m_deathExplosion.spawnAngle = 6.28f;

            //m_deathExplosion.startStretch = new Particles.Range(1.0f, 5.0f);
            //m_deathExplosion.endStretch = new Particles.Range(20.0f, 30.0f);

            m_deathExplosion.lifeTime = new Particles.Range(2.2f, 2.2f);
            m_deathCooldown = 2.0f;

            m_deathExplosion.startSpeed = new Particles.Range(70.0f, 70.0f);
            m_deathExplosion.endSpeed = new Particles.Range(5.0f, 5.0f);
            #endregion
        ...
        {
            if( !m_dying )
            sb.Draw(bulletSprite, m_position, new Rectangle(0, 0, 8, 8), Color.Violet, m_rotationAngle, m_origin, m_spriteScale, SpriteEffects.None, 0.0f);
            //int bw = 2;
    ...
    public class ConceptMissileEnemy
    {
        ...
        public ConceptMissileEnemy(Vector2 position)
        {
           ...
           //m_deathExplosion = Particles.ParticleManager.CreateParticleSystem(Particles.ParticleManager.defaultParticle);
           //
           //m_deathExplosion.rate = 0.01f;
           ////m_deathExplosion.startColor = Color.Orange;
           ...
           //m_deathCooldown = 2.0f;            
           //
           //m_deathExplosion.startSpeed = new Particles.Range(10.0f, 500.0f);
    ...
    public class Concept : AIE.IGameState
    {
        ...
        Texture2D m_redExplosionSpriteSheet;
        TextureAtlas.AnimatedSprite m_redExplosionAnimation;
        ...
        public override void Update(GameTime gT)
        {
            ...
            if (explode)
                m_redExplosionAnimation.Update();
            ...
            for (int iter = 0; iter < m_missleEnemyList.Count(); iter++)
            {
                ...
                if (m_missleEnemyList[iter].GetPosition().X >= Globals.m_windowWidth || m_missleEnemyList[iter].GetPosition().X <= 0 || m_missleEnemyList[iter].GetPosition().Y >= Globals.m_windowHeight || m_missleEnemyList[iter].GetPosition().Y <= 0)
                {
                    m_missleEnemyList[iter].Explode();
                    explode = true;
                    m_redExplosionAnimation.ResetAnimation();
                    m_redExplosionAnimation.m_position = m_missleEnemyList[iter].GetPosition();
                }
            }            
        ...
        public override void Draw(GameTime gT, SpriteBatch sb)
        {
            ...
            if (explode)
                m_redExplosionAnimation.Draw(sb);

            sb.DrawString(Globals.m_defaultFont, m_redExplosionAnimation.m_position.ToString(), Vector2.Zero, Color.White);
