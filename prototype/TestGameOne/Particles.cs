...
namespace Particles
{
    ...
    public class ParticleSystem
    {
        ...
        private int particleToUpdate = 0;
        ...
        private void UpdateParticles(float dt)
        {
            for (int i = 0; i < 15; ++i)
            {
                ...
                if (particleToUpdate < particles.Count)
                {
                    particles[particleToUpdate].Update(dt, deltaPosition, deltaRotation, virgin ? false : localTranslate);

                    if (!particles[particleToUpdate].alive)
                    {
                        particles.RemoveAt(particleToUpdate--);
                    }
                }
